...
class PlayState extends MusicBeatState
{
	...
	public var perfectsTxt:FlxText;
	...
	var freeCandy:FlxButton;

	var fecha:FlxButton;

	var perfectTween:FlxTween;
	...
	var perfectColorTween:FlxTween;
	var sickColorTween:FlxTween;
	var goodColorTween:FlxTween;
	var badColorTween:FlxTween;
	var shitColorTween:FlxTween;
	var comboColorTween:FlxTween;
	...
	var fazIssoDark:FlxSprite;
	var fazIssoTexto:FlxText;
	var fazIssoImagem:FlxSprite;
	...
	override public function create()
	{
		...
		if (ClientPrefs.scoreTxtType == 'Classic') {
			ratingStuff = [
		...
		if (ClientPrefs.timeBarType == 'Cool Time Bar') {
		...
		msTimeTxt.velocity.y -= FlxG.random.int(140, 175);
		add(msTimeTxt);
		...
		if(ClientPrefs.timeBarType == 'Cool Time Bar') 
		{
			timeTxt.size = (timeTxt.width >= 4 ? 20 : 24); //if width is more then 2 the size is 20, if not the size is 24
			timeTxt.y += 3;
		}
		...
		healthBarOverlay = new AttachedSprite('healthBar');
		...
		if (ClientPrefs.scoreTxtType == 'Classic' || ClientPrefs.scoreTxtType == 'Cooler Score') {
			scoreTxt.size = 20;
			scoreTxt.y += 14;
		}
		...
		add(songTxt);
		songTxt.text = curSong + " - " + storyDifficultyText.toUpperCase() + " | OS " + MainMenuState.osEngineVersion;
		...
		perfectsTxt = new FlxText(12, FlxG.height/2, 0, "", 20);
		perfectsTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		perfectsTxt.scrollFactor.set();
		perfectsTxt.borderSize = 1.25;
		if(!ClientPrefs.removePerfects) add(perfectsTxt);

		sicksTxt = new FlxText(perfectsTxt.x, (!ClientPrefs.removePerfects ? perfectsTxt.y + 20 : FlxG.height/2), 0, "", 20);
		sicksTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		sicksTxt.scrollFactor.set();
		sicksTxt.borderSize = 1.25;
		add(sicksTxt);

		goodsTxt = new FlxText(sicksTxt.x, sicksTxt.y + 20, 0, "", 20);
		goodsTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		goodsTxt.scrollFactor.set();
		goodsTxt.borderSize = 1.25;
		add(goodsTxt);

		badsTxt = new FlxText(goodsTxt.x, goodsTxt.y + 20, 0, "", 20);
		badsTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		badsTxt.scrollFactor.set();
		badsTxt.borderSize = 1.25;
		add(badsTxt);

		shitsTxt = new FlxText(badsTxt.x, badsTxt.y + 20, 0, "", 20);
		shitsTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		shitsTxt.scrollFactor.set();
		shitsTxt.borderSize = 1.25;
		add(shitsTxt);

		comboTxt = new FlxText(shitsTxt.x, shitsTxt.y + 20, 0, "", 20);
		comboTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		comboTxt.scrollFactor.set();
		comboTxt.borderSize = 1.25;
		add(comboTxt);

		pressSpace = new FlxText(FlxG.width/2, FlxG.height/2, 0, "Press space now!!!", 20);
		...
		freeCandy = new FlxButton(healthBarBG.x + 500, healthBarBG.y + 10, "Free Candy", fazIsso);
		freeCandy.setGraphicSize(Std.int(freeCandy.width * 0.6), Std.int(freeCandy.height * 0.6));
		freeCandy.cameras = [camOther];
		//add(freeCandy);

		fazIssoDark = new FlxSprite().makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.BLACK);
		fazIssoDark.alpha = 0;
		fazIssoDark.scrollFactor.set();
		freeCandy.cameras = [camOther];
		add(fazIssoDark);

		fazIssoImagem = new FlxSprite().loadGraphic(Paths.image('contas'));
		fazIssoImagem.screenCenter();
		fazIssoImagem.y += 70;
		fazIssoImagem.alpha = 0;
		freeCandy.cameras = [camOther];
		fazIssoImagem.scrollFactor.set();
		add(fazIssoImagem);

		fazIssoTexto = new FlxText(fazIssoImagem.x, fazIssoImagem.y + 30, 0, "Parabéns, Você acabou de receber uma lição de casa de engenharia quantica", 16);
		fazIssoTexto.scrollFactor.set();
		fazIssoTexto.alpha = 0;
		freeCandy.cameras = [camOther];
		fazIssoTexto.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(fazIssoTexto);

		fecha = new FlxButton(fazIssoImagem.x, fazIssoImagem.y - 50, "fecha", fechaIsso);
		fecha.alpha = 0;
		freeCandy.setGraphicSize(Std.int(fecha.width + 0.3), Std.int(fecha.height + 0.3));
		freeCandy.cameras = [camOther];
		add(fecha);
	...
	function fazIsso() {
		...
		FlxTween.tween(fazIssoDark, {alpha: 0.5}, 0.15);
		FlxTween.tween(fazIssoTexto, {alpha: 1}, 0.15);
		FlxTween.tween(fazIssoImagem, {alpha: 1}, 0.15);
		FlxTween.tween(fecha, {alpha: 1}, 0.15);
		freeCandy.alpha = 0; fecha.alpha = 1;
	}

	function fechaIsso() {
		instance.paused = false;
		fazIssoDark.alpha = 0; fazIssoTexto.alpha = 0; fazIssoImagem.alpha = 0; fecha.alpha = 0; freeCandy.alpha = 1;
	}
  ...
  public function clearShaderFromCamera(cam:String){
	...
	public function updateScore(miss:Bool = false)
	{
		...
		if (ClientPrefs.scoreTxtType == 'New Score') {
			scoreTxt.text = 'Score: ' + songScore 
			+ ' | Combo Breaks: ' + songMisses 
			+ ' | Average: ' + (ratingName == '?' ? '?' : Math.round(averageMs) + 'ms')
			+ ' | Accuracy: ' + ratingName
			+ (ratingName != '?' ? ' (${convertedAccDisplay}%) [$ratingFC]' : '');
		}
		...
		if (ClientPrefs.scoreTxtType == 'Cooler Score') { //i will some day make a better name
			scoreTxt.text = 'Score: ' + songScore
			+ ' | Combo Breaks: ' + songMisses
			+ ' | Accuracy: ' + convertedAccDisplay + '%' + (songMisses > 0 ? '' : ' - $ratingFC');
		}
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			if (pressSpaceTimer != null)
				pressSpaceTimer.active = false;
	...
	function spaceText(shouldItPlay:Bool = true) {
		if (shouldItPlay) {
			pressSpaceTimer = new FlxTimer().start(0.5, function(tmr:FlxTimer)
			{
				pressSpace.alpha = 1;
				pressSpaceTimer = new FlxTimer().start(0.5, function(tmr:FlxTimer)
				{
	...
	override public function update(elapsed:Float)
	{
		...
		{
			if (bfsection && (boyfriend.animation.curAnim.name == null || (StringTools.contains(boyfriend.animation.curAnim.name, "idle")
			...
			if (!bfsection && ((dad.animation.curAnim.name == null || StringTools.contains(dad.animation.curAnim.name, "idle"))
		...
		if (generatedMusic && (!isCameraOnForcedPos && !followChars))
		{
			...
			if (!bfsection && !SONG.notes[Math.floor(curStep / 16)].gfSection)
			{
				isCameraOnForcedPos = false;
				moveCameraSection(Std.int(curStep / 16));
			}
		...
		/*if (SONG.song.toLowerCase() == 'dad battle' && curStep == 50) {
			spaceText();
		} if (SONG.song.toLowerCase() == 'dad battle' && curStep == 80) {
			spaceText(false);
		}*/
		...
		if (ClientPrefs.stuff) {
			perfectsTxt.text = 'Perfects: ' + perfects;
	...
	function tweenPerfect():Void {
		if(perfectTween != null) {
			perfectTween.cancel();
		}
		perfectsTxt.scale.x = 1.03;
		perfectsTxt.scale.y = 1.03;
		perfectTween = FlxTween.tween(perfectsTxt.scale, {x: 1, y: 1}, 0.2, {
			onComplete: function(twn:FlxTween) {
				perfectTween = null;
			}
		});
	...
	function tweenSick():Void {
		if(sickTween != null) {
			sickTween.cancel();
		}
		sicksTxt.scale.x = 1.03;
		sicksTxt.scale.y = 1.03;
		sickTween = FlxTween.tween(sicksTxt.scale, {x: 1, y: 1}, 0.2, {
			onComplete: function(twn:FlxTween) {
				sickTween = null;
			}
		});
	...
	function tweenGood():Void {
		if(goodTween != null) {
			goodTween.cancel();
		}
		goodsTxt.scale.x = 1.03;
		goodsTxt.scale.y = 1.03;
		goodTween = FlxTween.tween(goodsTxt.scale, {x: 1, y: 1}, 0.2, {
			onComplete: function(twn:FlxTween) {
				goodTween = null;
			}
		});
	...
	function tweenBad():Void {
		if(badTween != null) {
			badTween.cancel();
		}
		badsTxt.scale.x = 1.03;
		badsTxt.scale.y = 1.03;
		badTween = FlxTween.tween(badsTxt.scale, {x: 1, y: 1}, 0.2, {
			onComplete: function(twn:FlxTween) {
				badTween = null;
			}
		});
	...
	function tweenShit():Void {
		if(shitTween != null) {
			shitTween.cancel();
		}
		shitsTxt.scale.x = 1.03;
		shitsTxt.scale.y = 1.03;
		shitTween = FlxTween.tween(shitsTxt.scale, {x: 1, y: 1}, 0.2, {
			onComplete: function(twn:FlxTween) {
				shitTween = null;
			}
		});
	...
	function tweenCombo():Void {
		if(comboTween != null) {
			comboTween.cancel();
		}
		comboTxt.scale.x = 1.03;
		comboTxt.scale.y = 1.03;
		comboTween = FlxTween.tween(comboTxt.scale, {x: 1, y: 1}, 0.2, {
			onComplete: function(twn:FlxTween) {
				comboTween = null;
			}
		});
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if (!note.isSustainNote) {
			if (daRating.perfect) {
				tweenPerfect();
				perfectColorTween = FlxTween.color(perfectsTxt, 0.25, 0xFF56d76d, FlxColor.WHITE);
			} if (daRating.sick) {
				tweenSick();
				sickColorTween = FlxTween.color(sicksTxt, 0.25, 0xFF56d76d, FlxColor.WHITE);
			} if (daRating.good) {
				tweenGood();
				goodColorTween = FlxTween.color(goodsTxt, 0.25, 0xFF56d76d, FlxColor.WHITE);
			} if (daRating.bad) {
				tweenBad();
				badColorTween = FlxTween.color(badsTxt, 0.25, 0xFF56d76d, FlxColor.WHITE);
			} if (daRating.shit) {
				tweenShit();
				shitColorTween = FlxTween.color(shitsTxt, 0.25, 0xFF56d76d, FlxColor.WHITE);
			}
		}

		if (ClientPrefs.betterHealthSystem == true) {
			if (daRating.perfect) { health += 0.035; }
			if (daRating.sick) { health += 0.03; }
			if (daRating.good) { health += 0.023; }
			if (daRating.bad) { health -= 0.02; }
			if (daRating.shit) { health -= 0.035; }
		}
			...
			if(char != null)
			{
				if (!char.specialAnim)
				{
					...
					if (!bfsection && followChars)
					{
						...
						if (SONG.notes[curSection].gfSection) {
							xx = (gf.getMidpoint().x + camerashit[0]) + gf.cameraPosition[0] + girlfriendCameraOffset[0];
							yy = (gf.getMidpoint().y + camerashit[1]) + gf.cameraPosition[1] + girlfriendCameraOffset[1];
						} else {
							xx = (dad.getMidpoint().x + camerashit[0]) + dad.cameraPosition[0] + opponentCameraOffset[0];
							yy = (dad.getMidpoint().y + camerashit[1]) + dad.cameraPosition[1] + opponentCameraOffset[1];
						}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote)
			{
				combo += 1;
				tweenCombo();
				comboColorTween = FlxTween.color(comboTxt, 0.25, 0xFF56d76d, FlxColor.WHITE);
				if(combo > 9999) combo = 9999;
				popUpScore(note);
				if (ClientPrefs.betterHealthSystem != true) health += note.hitHealth * healthGain;
			}
				...
				else
				{
					if (!boyfriend.specialAnim)
					{
						...
						if (bfsection && followChars)
						{
							...
							xx = (boyfriend.getMidpoint().x - 100);
							yy = (boyfriend.getMidpoint().y - 100);
							xx -= boyfriend.cameraPosition[0] - boyfriendCameraOffset[0];
							yy += boyfriend.cameraPosition[1] + boyfriendCameraOffset[1];
